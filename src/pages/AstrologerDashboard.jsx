...
import { FaYoutube, FaTelegram, FaWhatsapp, FaInstagram, FaEdit, FaTrash, FaCalendarAlt, FaVideo, FaUsers, FaStar, FaUser, FaNewspaper, FaShoppingBag, FaFolder, FaFolderPlus, FaFolderOpen, FaFileAlt, FaDownload, FaPlus, FaBook, FaSave, FaTimes, FaExternalLinkAlt, FaCloudUploadAlt, FaCheck, FaBan, FaEnvelope } from 'react-icons/fa'
...
const mapBookingToConsultation = (booking) => ({
  id: booking.id,
  ...
  phoneNumber: booking.phoneNumber || booking.clientPhone,
  notes: booking.notes,
...
const getSpecialistIdsForUser = (user, specialists) => {
  const ids = new Set()
  if (!user) return ids

  if (user.id != null) ids.add(String(user.id))
  if (user.specialistId != null) ids.add(String(user.specialistId))

  ;(specialists || []).forEach(specialist => {
    const sameId = user.id != null && String(specialist.id) === String(user.id)
    const sameEmail = user.email && specialist.email &&
      String(specialist.email).toLowerCase().trim() === String(user.email).toLowerCase().trim()
    const sameName = user.name && specialist.name &&
      String(specialist.name).trim() === String(user.name).trim()

    if (sameId || sameEmail || sameName) {
      ids.add(String(specialist.id))
    }
  })
...
const AstrologerDashboard = () => {
  ...
  const { specialists, updateSpecialist, deleteSpecialist } = useSpecialists()
  const { 
  ...
  const [consultationActionLoading, setConsultationActionLoading] = useState(false)
  const [contactConsultation, setContactConsultation] = useState(null)
  const [contactMessage, setContactMessage] = useState('')
  const [rescheduleConsultation, setRescheduleConsultation] = useState(null)
  ...
  const loadConsultations = useCallback(async () => {
    ...
    if (!API_BASE_URL) {
      console.warn('VITE_API_URL не настроен')
      setConsultations([])
      return
    }

    try {
      const specialistIds = getSpecialistIdsForUser(user, specialists)
      let bookings = []

      for (const specialistId of specialistIds) {
        const response = await fetch(`${API_BASE_URL}/api/bookings?specialistId=${encodeURIComponent(specialistId)}`)
        if (response.ok) {
      ...
      if (bookings.length === 0) {
        ...
        if (response.ok) {
          ...
          bookings = allBookings.filter(booking =>
            specialistIds.has(String(booking.specialistId))
          )
        }
      }
      ...
      const consultationsData = uniqueBookings.map(mapBookingToConsultation)
  ...
  const handleConfirmBooking = async (bookingId) => {
    const API_BASE_URL = import.meta.env.VITE_API_URL || ''
    if (!API_BASE_URL) {
      alert('VITE_API_URL не настроен. Невозможно подтвердить запись.')
      return
    }

    setConsultationActionLoading(true)
    try {
      const response = await fetch(`${API_BASE_URL}/api/bookings/${encodeURIComponent(bookingId)}/confirm`, {
        method: 'PUT',
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Сервер ответил ${response.status}`)
      }

      setConsultations(prev =>
        prev.map(c => (c.id === bookingId ? { ...c, status: 'upcoming' } : c))
      )
  ...
  const handleCancelBooking = async (bookingId) => {
    if (!window.confirm('Вы уверены, что хотите отменить эту консультацию?')) {
      return
    }

    const API_BASE_URL = import.meta.env.VITE_API_URL || ''
    if (!API_BASE_URL) {
      alert('VITE_API_URL не настроен. Невозможно отменить запись.')
      return
    }

    setConsultationActionLoading(true)
    try {
      const response = await fetch(`${API_BASE_URL}/api/bookings/${encodeURIComponent(bookingId)}/cancel`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' }
      })
      ...
      setConsultations(prev =>
        prev.map(c => (c.id === bookingId ? { ...c, status: 'cancelled' } : c))
      )
      await loadConsultations()
    } catch (error) {
  ...
  const openRescheduleModal = (consultation) => {
    ...
    setRescheduleDate(consultation.date || '')
    setRescheduleTime(consultation.time || '')
  }
  ...
  const handleRescheduleBooking = async (event) => {
    ...
    if (!rescheduleConsultation) return

    const API_BASE_URL = import.meta.env.VITE_API_URL || ''
    if (!API_BASE_URL) {
      alert('VITE_API_URL не настроен. Невозможно перенести запись.')
      return
    }

    if (!rescheduleDate || !rescheduleTime) {
      alert('Укажите новую дату и время')
      return
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/bookings/${encodeURIComponent(rescheduleConsultation.id)}/reschedule`,
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
      ...
      await loadConsultations()
      alert('Консультация перенесена. Клиент увидит новое время в своём кабинете.')
    } catch (error) {
  ...
  const openContactModal = (consultation) => {
    setContactConsultation(consultation)
    setContactMessage('')
  }

  const closeContactModal = () => {
    setContactConsultation(null)
    setContactMessage('')
  }

  const handleSendClientMessage = async (event) => {
    event.preventDefault()
    if (!contactConsultation || !contactMessage.trim()) return

    const API_BASE_URL = import.meta.env.VITE_API_URL || ''
    if (!API_BASE_URL) {
      alert('VITE_API_URL не настроен. Невозможно отправить сообщение.')
      return
    }
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/bookings/${encodeURIComponent(contactConsultation.id)}/messages`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            fromUserId: user?.id,
            fromUserName: user?.name || profile.name,
      ...
      closeContactModal()
      alert(`Сообщение отправлено клиенту ${contactConsultation.clientName}`)
    } catch (error) {
  ...
  const renderConsultationActions = (consultation) => {
    const isPending = consultation.status === 'pending'
    const isUpcoming = consultation.status === 'upcoming'
    const disabled = consultationActionLoading

    if (!isPending && !isUpcoming) return null
    ...
    return (
      <div style={{ display: 'flex', gap: '10px', marginTop: '15px', flexWrap: 'wrap' }}>
        ...
        <button
          type="button"
          className="btn btn-secondary"
          style={{ border: '1px solid #2196F3', color: '#2196F3', background: 'transparent' }}
          disabled={disabled}
    ...
    <div>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, color: '#333' }}>
                Сообщение клиенту: {contactConsultation.clientName}
              </h3>
              <button type="button" className="btn btn-secondary" onClick={closeContactModal} style={{ padding: '8px 12px' }}>
                <FaTimes />
              </button>
            </div>
            <p style={{ color: '#666', marginBottom: '16px', fontSize: '14px' }}>
              Консультация {contactConsultation.date} в {contactConsultation.time}. Клиент увидит ваше сообщение в личном кабинете.
            </p>
            <form onSubmit={handleSendClientMessage}>
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                ...
                <button
                  type="submit"
                  className="btn btn-primary"
                  disabled={consultationActionLoading || !contactMessage.trim()}
                >
                  Отправить
                </button>
              </div>
      ...
      {rescheduleConsultation && (
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, color: '#333' }}>Перенести консультацию</h3>
              <button type="button" className="btn btn-secondary" onClick={closeRescheduleModal} style={{ padding: '8px 12px' }}>
                <FaTimes />
              </button>
            </div>
            <p style={{ color: '#666', marginBottom: '16px', fontSize: '14px' }}>
              Клиент: {rescheduleConsultation.clientName}
            </p>
            <form onSubmit={handleRescheduleBooking}>
              ...
              <div className="form-group" style={{ marginBottom: '20px' }}>
                <label htmlFor="reschedule-time">Новое время</label>
                <input
                  id="reschedule-time"
                  type="time"
              ...
              <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                ...
                <button type="submit" className="btn btn-primary" disabled={consultationActionLoading}>
                  Сохранить
                </button>
              </div>
            </form>
