    ...
    const loadConsultations = async () => {
      ...
      if (API_BASE_URL) {
        try {
          ...
          const response = await fetch(`${API_BASE_URL}/api/bookings?clientId=${encodeURIComponent(user.id)}`)
              ...
              consultationsData.map(async (consultation) => {
                try {
                  const messagesResponse = await fetch(
                    `${API_BASE_URL}/api/bookings/${encodeURIComponent(consultation.id)}/messages`
                  )
                  if (messagesResponse.ok) {
                    messagesMap[consultation.id] = await messagesResponse.json()
                  }
                } catch (messagesError) {
                  console.warn('Не удалось загрузить сообщения:', messagesError)
                }
              })
            )
            setConsultationMessages(messagesMap)
      ...
      <div key={consultation.id} className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
          <div style={{ flex: 1 }}>
            ...
            {consultationMessages[consultation.id]?.length > 0 && (
              <div style={{
                marginTop: '15px',
                padding: '15px',
                background: '#e8f4fd',
                ...
                <p style={{ color: '#1565c0', fontWeight: 'bold', marginBottom: '10px' }}>
                  Сообщения от астролога
                </p>
                {consultationMessages[consultation.id].map(msg => (
                  <div key={msg.id} style={{ marginBottom: '10px' }}>
                    <p style={{ color: '#333', marginBottom: '4px' }}>{msg.message}</p>
                    <p style={{ color: '#666', fontSize: '12px' }}>
                      {msg.fromUserName} · {new Date(msg.createdAt).toLocaleString('ru-RU')}
                    </p>
                  </div>
                ))}
              </div>
